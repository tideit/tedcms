...
abstract class BaseWechat
{
	...
	public function valid($return=false)
    {
        ...
        } elseif (isset($_GET["echostr"])) {   // 接入验证
    ...
    protected function log($log){
    		if ($this->debug && function_exists($this->logcallback)) {
    			if (is_array($log)) $log = print_r($log,getRevtrue);
	...
	*/
	...
	public function text($text='')
	{
		...
		//@file_put_contents(__DIR__.'/../../mobile/web/test.log',json_encode($msg));
		$this->Message($msg);
		return $this;
	}
	/**
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
        ...
        $redis->auth('foobared');
        $result = $redis->setex($cachename,$expired,$value);
        return $result;
	}
	...
	public function getJsCardTicket($appid='',$api_ticket=''){
		...
		$authname = 'wechat_api_ticket_wxcard_'.$appid;
		if ($rs = $this->getCache($authname))  {
			$this->api_ticket = $rs;
			return $rs;
		}
	...
	public function getUserInfo($openid){
		...
		//file_put_contents(Yii::getAlias('@mobile').'/runtime/app.log', $result);
		if ($result)
		{
			$json = json_decode($result,true);
			if (isset($json['errcode'])) {
...
class Wechat extends BaseWechat
{
    ...
    public function __construct($options)
    {
        $this->cachedir = isset($options['cachedir']) ? dirname($options['cachedir'].'/.cache') . '/' : \Yii::getAlias("@source").'/temp/';
