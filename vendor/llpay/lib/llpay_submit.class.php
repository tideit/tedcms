...
class LLpaySubmit {
	 ...
	 * 退款网关
	 * */
	var $llpay_refund_gateway = "https://yintong.com.cn/traderapi/refund.htm";
	...
	function buildRequestMysign($para_sort) {
		//ios & android所有参与订单签名的字段，这些字段以外不参与签名
		$toSignParams = $para_sort;
		if(isset($para_sort['app_request']) && $para_sort['app_request'] != '3') {
			$needSignKeysForApp = ["busi_partner", "dt_order", "info_order",
					"money_order", "name_goods", "no_order", "notify_url",
					"oid_partner","risk_item", "sign_type", "valid_order"];
			$toSignParams = [];
			foreach( $para_sort as $key => $value ) {
				if(in_array($key, $needSignKeysForApp)) {
					$toSignParams[$key] = $value;
				}
			}
		}
		//把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
		$prestr = createLinkstring($toSignParams);
		$mysign = "";
                //PHP5.3 版本以上 风控参数去斜杠
...
// 		return urldecode(json_encode($para_sort));
	...
	function buildRequestParaToString($para_temp) {
		...
		$para = rawurldecode(json_encode($para));
		return $para;
...
// 		return $request_data;
	 ...
	 * @return 连连支付处理结果
	 */
	function buildRequestHttp($para_temp, $gate=null) {
		$sResult = '';
		...
		//远程获取数据
		$sResult = getHttpResponsePOST($gate ? $gate : $this->llpay_gateway_new, $this->llpay_config['cacert'], $request_data, trim(strtolower($this->llpay_config['input_charset'])));
	 ...
	 * @return 连连支付处理结果
	 */
	function buildRequestJSON($para_temp, $gateway) {
	    $sResult = '';
	    ...
	    //远程获取数据
	    $sResult = getHttpResponseJSON($gateway, $request_data);
