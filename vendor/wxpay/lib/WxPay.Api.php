...
//        exit;
	...
	public static function redpack($inputObj, $timeOut = 6)
	{
	    ...
	    if(!$inputObj->IsMch_billnoSet()) {
	        throw new WxPayException("发放红包接口中，缺少必填参数mch_billno！");
	    }else if(!$inputObj->IsSend_nameSet()){
	        throw new WxPayException("发放红包接口中，缺少必填参数send_name！");
	    }else if(!$inputObj->IsRe_openidSet()){
	        throw new WxPayException("发放红包接口中，缺少必填参数re_openid！");
	    }else if(!$inputObj->IsTotal_amountSet()){
	        throw new WxPayException("发放红包接口中，缺少必填参数total_amount！");
	    }else if(!$inputObj->IsTotal_numSet()){
	        throw new WxPayException("发放红包接口中，缺少必填参数total_num！");
	    }else if(!$inputObj->IsWishingSet()){
	        throw new WxPayException("发放红包接口中，缺少必填参数wishing！");
	    }else if(!$inputObj->IsAct_nameSet()){
	        throw new WxPayException("发放红包接口中，缺少必填参数act_name！");
	    }else if(!$inputObj->IsRemarkSet()){
	        throw new WxPayException("发放红包接口中，缺少必填参数remark！");
	    }
	    $inputObj->SetAppid(WxPayConfig::$APPID);//公众账号ID
	    $inputObj->SetMch_id(WxPayConfig::$MCHID);//商户号
	    $inputObj->SetClient_ip(self::getServerIp());//终端ip
	    $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
	{
		...
		if($data){
			...
			$error = curl_errno($ch);
			curl_close($ch);
			throw new WxPayException("curl出错，错误码:$error".WxPayConfig::$SSLCERT_PATH);
		}
	}
